...
suspend fun initPayment(auth: Authentication, session: Session, amount: Int): Payment {
  ...
  request.setJSON(
      buildJsonArray {
        addJsonObject {
          putJsonObject("paiementPayline") { putJsonObject("hote") { put("id", session.hostID) } }
          put("montant", amount)
        }
      })
  ...
  return Payment(
      ...
      cancelURL =
          "https://espacenumerique.turbo-self.com/PagePaiementRefuse.aspx?token=" +
              content.jsonObject["token"]!!.jsonPrimitive.content,
      returnURL =
          "https://espacenumerique.turbo-self.com/PagePaiementValide.aspx?token=" +
              content.jsonObject["token"]!!.jsonPrimitive.content,

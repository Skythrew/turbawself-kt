...
fun decodeBookingDay (bookingDay: JsonObject, weekRange: WeekRange): BookingDay {
    ...
    val date = weekRange.from.toInstant(TimeZone.currentSystemDefault()).plus((dayOfWeek - 1).toDuration(DurationUnit.DAYS)).toLocalDateTime(TimeZone.currentSystemDefault())

    return BookingDay(
        id = if (bookingDay["web"] != null) bookingDay["web"]!!.jsonObject["id"]!!.jsonPrimitive.content else null,
